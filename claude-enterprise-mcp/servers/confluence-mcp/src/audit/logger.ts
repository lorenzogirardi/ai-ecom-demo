...
/**
 * Audit logger wrapper for Confluence MCP tools
 */
export class ConfluenceAuditLogger {
  ...
  async log(entry: Omit<AuditLog, 'timestamp' | 'tool'>): Promise<void> {
    ...
    try {
      ...
      const logFile = `${logDir}/confluence-${new Date().toISOString().split('T')[0]}.jsonl`;
...
export function createAuditLogger(config: AuditLoggerConfig): ConfluenceAuditLogger {
  return new ConfluenceAuditLogger(config);
}
