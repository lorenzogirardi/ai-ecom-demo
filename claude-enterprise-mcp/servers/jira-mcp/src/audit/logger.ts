...
/**
 * Audit logger wrapper for Jira MCP tools
 */
export class JiraAuditLogger {
  ...
  async log(entry: Omit<AuditLog, 'timestamp' | 'tool'>): Promise<void> {
    ...
    try {
      ...
      const logFile = `${logDir}/jira-${new Date().toISOString().split('T')[0]}.jsonl`;
...
export function createAuditLogger(config: AuditLoggerConfig): JiraAuditLogger {
  return new JiraAuditLogger(config);
}
