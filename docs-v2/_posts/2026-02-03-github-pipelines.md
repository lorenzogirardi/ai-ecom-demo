...
order: 3
reading_time: 14
tags: [github-actions,ci-cd,argocd]
---


Diagrams of the CI/CD pipelines implemented in the project.
...
|----------|---------|---------|
| **Backend CI/CD** | Push/PR on `apps/backend/**` | Build, test, security scan |
| **Frontend CI/CD** | Push/PR on `apps/frontend/**` | Build, test, Lighthouse, security scan |
| **Infrastructure CI** | Push/PR on `infra/terraform/**`, `helm/**` | Lint, validate, security scan |
| **Deploy ArgoCD** | Manual (workflow_dispatch) | Deploy ArgoCD + Applications |
| **Load Test** | Manual (workflow_dispatch) | k6 performance testing |
...
sequenceDiagram
    ...
    Note over Dev,Repo: Trigger: Push/PR on apps/backend/**

    Dev->>GH: git push / PR
    GH->>GA: Trigger backend-ci-cd.yml

    par Parallel Jobs (Stage 1)
    ...
    alt All jobs passed
        Note over GA,ECR: Only on push (not PR)
        GA->>GA: Docker Build
...
sequenceDiagram
    ...
    Note over Dev,Repo: Trigger: Push/PR on apps/frontend/**
...
sequenceDiagram
    ...
    Note over Dev,Repo: Trigger: Push/PR on infra/terraform/**, helm/**
    ...
    Note over GA: terraform init<br/>terraform validate<br/>for each module
```
...
sequenceDiagram
    ...
    Note over Op,Argo: Trigger: Manual (workflow_dispatch)
...
sequenceDiagram
    ...
    Note over Op,App: Trigger: Manual (workflow_dispatch)
...
| Name | Type | Usage |
|------|------|-------|
| `AWS_ROLE_ARN` | Secret | OIDC role for AWS |
| `GITHUB_TOKEN` | Auto | Commits, artifacts |
...
|----------|----------|-----------|
| Backend CI | `trivy-backend-report` | 30 days |
| Frontend CI | `trivy-frontend-report` | 30 days |
| Frontend CI | `lighthouse-reports` | 30 days |
| Load Test | `k6-reports-*` | 30 days |
...
| File | Purpose |
|------|---------|
| `.checkov.yaml` | Skip rules for Checkov |
| `.tflint.hcl` | AWS plugin for TFLint |
| `.gitleaks.toml` | Allowlist for Gitleaks |
