...
Diagrammi dell'architettura AWS e delle interazioni tra i servizi.

---

## Panoramica Servizi AWS

| Categoria | Servizio | Scopo |
|-----------|----------|-------|
| **Compute** | EKS | Kubernetes cluster |
| **Compute** | EC2 (Node Group) | Worker nodes |
| **Network** | VPC | Rete isolata |
| **Network** | ALB | Load balancer applicativo |
| **Network** | NAT Gateway | Accesso internet per subnet private |
| **Network** | CloudFront | CDN globale |
| **Database** | RDS PostgreSQL | Database relazionale |
| **Cache** | ElastiCache Redis | Cache in-memory |
| **Storage** | S3 | Assets statici + Terraform state |
| **Storage** | ECR | Container registry |
| **Security** | Secrets Manager | Credenziali database/cache |
| **Security** | IAM | Ruoli e policy |
...
## 4. Security Groups e Flusso Rete
...
sequenceDiagram
    ...
    ESO->>SM: Poll for secrets<br/>(ogni 1h)
...
## 9. Costi Stimati (Demo Environment)
...
> **Nota:** Costi basati su us-east-1, ambiente demo con risorse minime.
> In produzione i costi aumentano significativamente (Multi-AZ RDS, nodi più grandi, ecc.)
